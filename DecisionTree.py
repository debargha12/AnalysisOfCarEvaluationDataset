...
#true_rows, false_rows = partition(training_data, Question(0,'Red','color'))
...
def entropy(rows):
    ...
    for lbl in counts:
        ...
        answer += - prob_of_lbl * log2(prob_of_lbl)
    return answer
...
#best_gain, best_question = find_best_split(training_data,header)
# FYI: is color == Red is just as good. See the note in the code above
# where I used '>='.
#######
#print(best_question)
#This finds the best information gain and the best question to ask
...
#my_tree = build_tree(training_data,header,0,0)
...
def computeAccuracy(rows, node):
    ...
    numAccurate = 0
    for row in rows :
        predictedLabel=classify(row,node)
        trueLabel=row[-1]
        for i,j in predictedLabel.items():
            predictedLabels=i
        if predictedLabels == trueLabel:
